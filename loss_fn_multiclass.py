    ...
    def forward(
        ...
        for batch in range(shape[0]):
            for c in range(shape[1]):
                if target_one_hot[batch, c, :, :].sum() != 0:
                    counted_targets.append(target_one_hot[batch, c, :, :])
                    counted_inputs.append(input[batch,c,:,:])
        counted_targets = tuple(counted_targets)
        counted_inputs = tuple(counted_inputs)
        
        final_targets = torch.stack(counted_targets, 0)
        final_targets = final_targets.to(torch.int64)
        final_inputs = torch.stack(counted_inputs,0)                    
        
        # compute softmax over the classes axis
        input_soft = F.softmax(final_inputs, dim=1) + self.eps

        # compute the actual focal loss
        weight = torch.pow(1. - input_soft, self.gamma)
        focal = -self.alpha * weight * torch.log(input_soft)
        loss_tmp = torch.sum(final_targets * focal, dim=1)
